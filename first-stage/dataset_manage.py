...
class ADNI2(Dataset): ### Modified from Tecent MedicalNet Pipeline
    def __init__(self, root_dir, input_D, input_H, input_W, phase, time:list, modality:list, PTID_m_list:list=None, image_transform=None, file=None):
        ...
        self.Spatial_transform = tio.OneOf({tio.RandomFlip(axes=2, flip_probability=0.5):0.25,
                                    tio.RandomAffine(scales=(0.9, 1.2), degrees=10, p=0.5):0.25,
                                    tio.RandomElasticDeformation(num_control_points=(10, 10, 10), locked_borders=2, p=0.5):0.25,
                                    tio.RandomSwap(patch_size=10, p=0.5):0.25})

        self.Intensity_transform = tio.OneOf({tio.RandomBiasField(coefficients=0.1,p=0.5):0.25,
                                        tio.RandomGamma(log_gamma=(-0.5, 0.5),p=0.5):0.25,
                                        tio.RandomNoise(p=0.5):0.25,
                                        tio.RandomBlur(p=0.5):0.25})
        ## modality: structured
    ...
    def __getitem__(self, idx):
        ...
        if self.image_transform != None:
            ...
            MRI = image_data[self.modality[0]]
            FDG = image_data[self.modality[1]]
            ...
            image_data[self.modality[0]] = subject.mri.numpy()
            image_data[self.modality[1]] = subject.fdg.numpy()
...
class ADNI2_pre(Dataset): ### Modified from Tecent MedicalNet Pipeline
    ...
    def __getitem__(self, idx):
        ...
        # read image and labels
        ith_info_pre = self.PTID_m_list[idx]
        condition = self.conditions[idx]
        #print('condition:    ',condition)
        mod_condition = torch.Tensor([condition[self.modality[0]], condition[self.modality[1]]])
        ...
        for mod in self.modality :
            ...
            if condition[mod] == 0 :
                ...
                image_data[mod] = Image
                continue
                #image_data[mod] = self.__nii2tensorarray__(Image)

            if mod != 'structured' :
                ith_info = ith_info_pre

                Image_name = os.path.join(self.root_dir, ith_info[0], 'processed',str(ith_info[1]),ith_info[2], mod + '.nii')
                if ith_info[2] in ['014_S_6199'] and mod == self.modality[0]:
                    print(Image_name)
                if not os.path.isfile(Image_name):
                    print('Image not found: ', Image_name, 'Condition: ', condition)
        ...
        return image_data, mod_condition
