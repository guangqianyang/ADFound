...
class ViT_register(nn.Module) :
    def __init__(self, image_size:list, patch_size, num_category, num_layers, num_heads, hidden_d, ffn_d, channels=3, dropout=0.0, attention_dropout=0.0, learnable_pe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6)) :
        ...
        self.register_tokens = nn.Parameter(
            torch.randn(num_register_tokens, hidden_d)
        )
        self.add_pos_embedding = nn.Parameter(torch.empty(1, 16, hidden_d).normal_(std=0.02))
...
class Text_encoder(nn.Module):
    def __init__(self, dropout_rate=0.2, use_layernorm=True):
        """
        A text encoder that generates 768-dimensional embedding features.
        Args:
            dropout_rate (float): Dropout rate for regularization.
            use_layernorm (bool): Whether to use LayerNorm instead of BatchNorm1d.
        """
        ...
        # Choose normalization: BatchNorm1d or LayerNorm
        if use_layernorm:
        ...
        # Activation function
        self.activation = nn.ReLU()  # You can replace with nn.GELU() if needed
    ...
    def forward(self, x):
        ...
        Returns:
            x: Output tensor of shape (batch_size, 768) after linear, activation, normalization, and dropout.
        """
        # Ensure input shape is (batch_size, 1)
        if x.dim() == 1:
            x = x.unsqueeze(1)  # Reshape (batch_size,) -> (batch_size, 1)

        x = self.fc2(x)          # Fully connected layer
        x = self.activation(x)   # Activation
        x = self.norm2(x)        # Normalization (BatchNorm1d or LayerNorm)
...
class ViM_multi(nn.Module) :
    def __init__(self, image_size:list, patch_size, num_category, num_layers, num_heads, hidden_d, ffn_d, channels=3, dropout=0.0, attention_dropout=0.0, learnable_pe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6)) :
        ...
        if learnable_pe :
            ...
            self.pos_embedding_tab = nn.Parameter(torch.empty(1, 8, hidden_d).normal_(std=0.02))
        else :
            if self.image_dim == 2 :
                self.pos_embedding = posemb_sincos_2d(self.image_size[0] // self.patch_size, self.image_size[1] // self.patch_size, self.hidden_d).cuda()
            else :
...
class ViM_MAE_multi(ViM_multi) :
    def __init__(self, image_size:list, patch_size, num_category, num_layers, num_heads, hidden_d, ffn_d, channels=3, dropout=0.0, attention_dropout=0.0, learnable_pe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), decoder_num_layers=1, decoder_num_heads=None, decoder_hidden_d=None, decoder_ffn_d=None, decoder_dropout=None, decoder_attention_dropout=None, decoder_norm_layer=None, mask_ratio=0.75) :
        ...
        self.text_encoder = Text_encoder()
        #self.mri_projection_c = nn.Linear(hidden_d, 512)
        #self.pet_projection_c = nn.Linear(hidden_d, 512)
        ...
        self.m = 0.999
        self.mri_token = nn.Parameter(torch.randn(1, 512, 768))
        self.pet_token = nn.Parameter(torch.randn(1, 512, 768))
        self.tab_token = nn.Parameter(torch.randn(1, 8, 768))
    ...
    def forward(self, x_mri, x_pet, tab, condition) :
        batch_size = x_mri.shape[0]
        ## [B, C, H, W, D]
        mri_condition = condition[:, 0]

        mri_mask = mri_condition.view(batch_size, 1, 1).expand(-1, 512, 768) 

        pet_condition = condition[:, 1]

        pet_mask = pet_condition.view(batch_size, 1, 1).expand(-1, 512, 768) 

        tab_condition = condition[:, 2]
        tab_mask = tab_condition.view(batch_size, 1, 1).expand(-1, 8, 768)
        ...
        #mri_token_expaned = self.mri_token.expand(batch_size, -1, -1)
        #pet_token_expaned = self.pet_token.expand(batch_size, -1, -1)
        tab_token_expaned = self.tab_token.expand(batch_size, -1, -1)

        #x_mri = x_mri * mri_mask + mri_token_expaned * (1 - mri_mask)
        #x_pet = x_pet * pet_mask + pet_token_expaned * (1 - pet_mask)
        x_tab = x_tab * tab_mask + tab_token_expaned * (1 - tab_mask)
        x_mri = x_mri + self.pos_embedding_mri
        x_pet = x_pet + self.pos_embedding_pet
        x_tab = x_tab + self.pos_embedding_tab
        ...
        x_unmasked = torch.concat([x_unmasked_mri, x_unmasked_pet, x_tab], dim=1)
        ## Go through the ViT encoder
        x_unmasked = self.encoder(x_unmasked)
        x_unmasked_mri = x_unmasked[:, :self.num_unmasked, :]
        x_unmasked_pet = x_unmasked[:, self.num_unmasked:2*self.num_unmasked, :]
        ...
        x_unmasked_c = torch.concat([x_unmasked_mri_c, x_unmasked_pet_c, x_tab], dim=1)
        ## Go through the ViT encoder
        ...
        x_unmasked_pet_c = x_unmasked_c[:, self.num_unmasked:2*self.num_unmasked, :]
...
if __name__ == "__main__" :
    print(posemb_sincos_3d(8, 8, 8, 384).cuda())
...
def vim_model(args) :
    ...
    elif name == 'vit_B_regnister' :
        model = ViT_register(image_size=[args.input_D, args.input_H, args.input_W], patch_size=8, num_category=3, num_layers=12, num_heads=12, hidden_d=768, ffn_d=3072, channels=1, learnable_pe=True).cuda()
    elif name == 'vim_L_mae' :
