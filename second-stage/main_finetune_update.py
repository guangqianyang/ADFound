...
parser.add_argument("--load_name", type=str, default="multi_B_mae_500")
...
def train(net, criterion, optimizer, train_loader, val_loader, test_loader, device, max_epoch, scheduler, writer, seed):
    ...
    for epoch in range(150):
        ...
        train_f1_score = f1_score(labels,predictions,average='weighted')
        labels_new = np.zeros((len(labels), 3))
        for i, value in enumerate(labels):
            labels_new[i, value] = 1
        train_auc = roc_auc_score(labels_new, y_scores, multi_class='ovr')
        ...
        for i, value in enumerate(labels):
            labels_new[i, value] = 1
        val_auc = roc_auc_score(labels_new, y_scores, multi_class='ovr')
        ...
        for i, value in enumerate(test_labels):
            labels_new[i, value] = 1
        test_auc = roc_auc_score(labels_new, test_y_scores, multi_class='ovr')
    ...
    torch.save(net.state_dict(), 'weights/'+args.model_type+'_seed'+str(seed)+'.pt')
