...
def main():
    ...
    train_loader, val_loader, test_loader = build_data_adni2_paired_data_v2(args.batch_size, args.input_D, args.input_H, args.input_W, workers=16)
    ...
    criterion = focal_loss(num_classes=3, gamma=3)
    ...
    train(net, criterion, optimizer, train_loader, val_loader, test_loader, device, max_epoch, scheduler, writer)
            
def train(net, criterion, optimizer, train_loader, val_loader, test_loader, device, max_epoch, scheduler, writer):
    best_acc = 0
    best_f1_score = 0
    best_train_acc = 0
    ...
    for epoch in range(max_epoch):
        ...
        labels_new = np.zeros((len(labels), 3))
        ...
        for i, value in enumerate(labels):
            labels_new[i, value] = 1
        val_auc = roc_auc_score(labels_new, y_scores, multi_class='ovo')
        ...
        labels_new = np.zeros((len(test_labels), 3))
        for i, value in enumerate(test_labels):
            labels_new[i, value] = 1
        test_auc = roc_auc_score(labels_new, test_y_scores, multi_class='ovo')
        ...
        if val_acc > best_acc:
            #torch.save(net.state_dict(), 'weights/best.pt')
            best_acc = val_acc
    ...
    print('The best validation accuracy is: {}'.format(best_train_acc))
    print('The best validation auc is: {}'.format(best_train_auc))
    print('The best validation f1 score is: {}'.format(best_train_f1_score))
