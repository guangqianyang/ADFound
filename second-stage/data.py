...
def build_data_adni2_paired_data(batch_size, input_D, input_H, input_W, workers=4) :
    root_dir = r'/data/krdu/dataset/ADNI/processed/'
    ...
    division_root = "/data/gqyang/AD_algorithm/Data_division/subjects/T1_FDG/three-class"
...
def build_data_adni2_paired_data_v2(batch_size, input_D, input_H, input_W, workers=4) :
    root_dir = r'/data/Data1/gqyang/dataset/ADFound_ori/finetune/ADNI2'
    csv_file = r'/data/zhyang/Alzheimer Prognosis/ADNI2.csv'

    #img_mod = 'PET_TregPET_SS'
    
    img_mod = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS', 'Tab']
    time = [0,3,6,12,24,36,48,60]
    ...
    val_id_list = pd.read_excel(os.path.join(division_root, 'val_data.xlsx'))['PTID_list'].values.tolist()
    test_id_list = pd.read_excel(os.path.join(division_root, 'test_data.xlsx'))['PTID_list'].values.tolist()
    tab = pd.read_csv('/data/Data2/gqyang/AD_algorithm/ADNI_TABLE_normalized.csv')
    ID_list = list(zip(tab['PTID'], tab['VISM_IN']))
    ...
    train_list_old = list(zip(train_id_list, train_m_list))
    val_list_old = list(zip(val_id_list, val_m_list))
    test_list_old = list(zip(test_id_list, test_m_list))

    train_list = []
    val_list = []
    test_list = []

    for patient in train_list_old:
        if patient not in ID_list:
            continue
        train_list.append(patient)

    for patient in val_list_old:
        if patient not in ID_list:
            continue
        val_list.append(patient)

    for patient in test_list_old:
        if patient not in ID_list:
            continue
...
def build_data_adni2_paired_data_pretrain(batch_size, input_D, input_H, input_W, workers=4) :
    ...
    root= '/data/Data1/gqyang/dataset/ADFound_ori/pretrain'
    img_mod = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS']
    time = [0,3,6,12,24,36,48,60]
    ...
    ID_list = list(zip(tab['COLPROT'], tab['VISM_IN'], tab['PTID']))
    ...
    all_data  = paired_data 
    all_conditions = paired_conditions

    Text_condition = []
    for i in range(len(all_data)):
        sample = all_data[i]
        if sample in ID_list:
            all_conditions[i]['Tab'] = 1
        else:
            all_conditions[i]['Tab'] = 0
            print('Tab data is not existed')
    ...
    modality = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS', 'Tab']
    trainset = ADNI2_pre(root_dir=root, input_D=input_D, input_H=input_H, input_W=input_W, phase='train',
                     time=time, modality=modality, PTID_m_list=all_data, conditions=all_conditions,image_transform=data_augmentation)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                               shuffle=True, num_workers=workers, drop_last=True)
    return train_loader
...
if __name__ == '__main__':
    ...
    train_loader = build_data_adni2_paired_data_pretrain(1, 128, 128, 128)
    '''
    i = 0
    modalities = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS']
    for image_data, condition in train_loader:
        T1 = image_data[modalities[0]]
        PET = image_data[modalities[1]]
        print('condition: ', condition )
        print('Sum of T1: ', torch.sum(T1))
        if condition[0][0] == 1:
            print('the maximum of T1:', torch.max(T1))
            print('the minimum of T1:', torch.min(T1))
    '''