...
for num_epochs in range(0, epochs + 1) :
    ...
    epoch_training_inter_loss =0
    
    for image_data, tab_data, condition in tqdm(train_loader) :
        optimizer.zero_grad()
        ...
        o1, o2, mask1, mask2, score_mri, score_pet,inter_score1, inter_score2 = model(x1, x2, tab, condition)
        ...
        inter_loss1 = F.cross_entropy(inter_score1, labels)
        inter_loss2= F.cross_entropy(inter_score2, labels)
        inter_loss = 0.5 * (inter_loss1 + inter_loss2) 

        loss_reg_mri = (loss0 + loss1)/2
        loss_reg_pet = (loss2 + loss3)/2
    
        loss = loss_mri + loss_pet + 0.05* (loss_reg_mri + loss_reg_pet)+ 0.05 * inter_loss
        ...
        epoch_training_inter_loss += inter_loss.detach().sum().item() * x1.shape[0]
    ...
    writer.add_scalar('Inter CL Loss', round(0.05*epoch_training_inter_loss / epoch_training_tot, 5), num_epochs)
