...
class ADNI2(Dataset): ### Modified from Tecent MedicalNet Pipeline
    def __init__(self, root_dir, input_D, input_H, input_W, phase, time:list, modality:list, PTID_m_list:list=None, image_transform=None, file=None):
        ...
        tab_new = pd.read_csv('/data/Data2/gqyang/AD_algorithm/ADNI_TABLE_normalized.csv')
        tab_new['PTID_VISM_IN'] = list(zip(tab_new['PTID'], tab_new['VISM_IN']))
        if PTID_m_list != None :
            tab_new = tab_new[tab_new['PTID_VISM_IN'].isin(PTID_m_list)]
        # remove temporal column PTID_VISM_IN
        # filtered = filtered.drop('PTID_VISM_IN', axis=1)
        tab_new = tab_new.set_index('PTID_VISM_IN')
        ...
        self.table = tab_new[['PTGENDER', 'PTEDUCAT', 'PTMARRY', 'APOE4', 'MMSE', 'ADNI_EF', 'ADNI_MEM', 'VISM_IN', 'AGE']]
    ...
    def __getitem__(self, idx):
        ...
        for mod in self.modality :
            if mod != 'Tab' :
                Image_name = os.path.join(self.root_dir, 'processed', str(ith_info[1]), ith_info[0], mod + '.nii')
                ...
                text_data = [tab['PTGENDER'], tab['PTEDUCAT'], tab['AGE']
                , tab['PTMARRY'], tab['APOE4'], tab['ADNI_EF']
                ,tab['ADNI_MEM'],tab['MMSE']]
                text_data = np.array(text_data).reshape(-1, 1)
                #print(text_data)
...
class ADNI2_pre(Dataset): ### Modified from Tecent MedicalNet Pipeline
    def __init__(self, root_dir, input_D, input_H, input_W, phase, time:list, modality:list, PTID_m_list:list=None, conditions: list=None, image_transform=None):
        super().__init__()
        self.root_dir = root_dir
        self.input_D = input_D
        self.input_H = input_H
        ...
        self.img_mod = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS']
        self.conditions_list = np.array([[condition[self.img_mod[0]], condition[self.img_mod[1]]] for condition in self.conditions])
        #print(self.conditions_list)
        num_T1 = self.conditions_list[:,0].sum()
        
        num_pet = self.conditions_list[:,1].sum()

        print('The number of T1 is ', num_T1)
        print('The number of PET is ', num_pet)

        tab = pd.read_csv('/data/Data2/gqyang/AD_algorithm/Data_division/Multi-modality/ADNI_TABLE_TOTAL_normalized.csv')
        tab['PTID_VISM_IN'] = list(zip(tab['COLPROT'], tab['VISM_IN'], tab['PTID']))
        if PTID_m_list != None :
            tab = tab[tab['PTID_VISM_IN'].isin(PTID_m_list)]
        # remove temporal column PTID_VISM_IN
        # filtered = filtered.drop('PTID_VISM_IN', axis=1)
    ...
    def __getitem__(self, idx):
        ...
        tab_data = None
        ith_info = self.PTID_m_list[idx]
        sample_condition = self.conditions[idx]
        for mod in self.modality :
            if mod != 'Tab' :
                if sample_condition[mod] == 1 :
                    ...
                    if self.image_transform != None :
                        Image = self.image_transform(Image)
                    image_data[mod] = Image

                else:
                    Image= np.zeros((1, 128, 128, 128))
                    Image = Image.astype("float32")
                    image_data[mod] = Image

            else :
                if sample_condition[mod] == 0 :
                    tab_data = torch.Tensor(np.zeros((8,1)))
                else:
                    tab = dict(self.table.loc[[ith_info], :].iloc[0])
        ...
        mod_condition = torch.Tensor([sample_condition[self.modality[0]], sample_condition[self.modality[1]],sample_condition[self.modality[2]]])

        return image_data, tab_data, mod_condition
