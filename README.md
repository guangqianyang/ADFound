...
- The first pre-training stage: The ADFound is pre-trained under the supervision of reconstruction loss for MRI $\mathcal L_{rec}^{M}$ and PET reconstruction $\mathcal L_{rec}^{P}$ and intra-modal contrastive loss for MRI $L_{intra}^{M}$ and PET $ L_{intra}^{P}$.
