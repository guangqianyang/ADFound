...
- The first pre-training stage: The ADFound is pre-trained under the supervision of reconstruction loss for MRI $\mathcal L_{rec}^{M}$ and PET reconstruction $\mathcal L_{rec}^{P}$ and intra-modal contrastive loss for MRI $ \mathcal L_{intra}^{M}$ and PET $\mathcal L_{intra}^{P}$.
