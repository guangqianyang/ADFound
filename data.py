...
def build_data_adni2_paired_data_v2(batch_size, input_D, input_H, input_W, workers=4) :
    root_dir = r'/data/Data1/gqyang/dataset/ADFound_ori/finetune/ADNI2/processed'
    csv_file = r'/data/zhyang/Alzheimer Prognosis/ADNI2.csv'
...
def build_data_adni2_paired_data_pretrain(batch_size, input_D, input_H, input_W, workers=4) :
    time = [0,3,6,12,18,24,36,48,60,72]
    root= '/data/Data1/gqyang/dataset/ADFound_ori/pretrain'
    ADNI = ['ADNI1','ADNI3','ADNIGO']
    img_mod = ['T1_TregT1_SS','PET-FDG_TregPET-FDG_SS']
    all_PTID_list = []
    for cohort in ADNI:
        root_dir = os.path.join(root, cohort,'processed')
        if not os.path.exists(root_dir):
            print(root_dir)
        else:
            print('folder is existed')
        #PTID_List1 = Image_Filter(time, ['PET_TregPET_SS'], True, root_dir)
        PTID_List1 = Image_Filter(time, ['PET-FDG_TregPET-FDG_SS'], True, root_dir)
        PTID_List2 = Image_Filter(time, ['T1_TregT1_SS'], True, root_dir)
        #print(len(PTID_List1))
        common_list = []
        for patient in PTID_List1:
            if patient in PTID_List2:
                cohort_PTID = [cohort, patient[1], patient[0]]
                common_list.append(cohort_PTID)
        random.shuffle(common_list)
        PTID_List = common_list
        all_PTID_list.extend(PTID_List)
    all_PTID_list.sort()
    #all_PTID_list = all_PTID_list[:100]
    #all_PTID_list = all_PTID_list[190:200]
    #all_PTID_list = [['ADNI3',0, '130_S_2403'],['ADNI3',24,'130_S_5258']]
    print('A total of {} patients for training a model'.format(len(all_PTID_list)))
    trainset = ADNI2_pre(root_dir=root, input_D=input_D, input_H=input_H, input_W=input_W, phase='train',
                     time=time, modality=img_mod, PTID_m_list=all_PTID_list, image_transform=data_augmentation)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                               shuffle=True, num_workers=workers, drop_last=True)
